...
def wrapper(f):
    def fun(l):
        ...
        # Step 1: standardize phone number
        # Step 2: Invoke the wrapped function, "f"
        for i in range(len(l)):
            l[i] = re.sub(r'.*(.{4})(.{4})', r'91 \1 \2', l[i])
            ''' Named group and backreferences:
            l[i] = re.sub(r'.*(?P<g1>.{4})(?P<g2>.{4})', r'91 \g<g1> \g<g2>', l[i])
            '''
        f(l)
